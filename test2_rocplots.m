...
%omegaMatrix = LGMLVQ_model.psis / norm(LGMLVQ_model.psis,'fro');

prototypes = LGMLVQ_model.w; 
...
for i=1:length(trainLab{1})
    ...
    for jk = 1:length(unique(trainLab{1}))
        omegaMatrix{jk} = LGMLVQ_model.psis{jk} ;
        dist(jk) = norm(omegaMatrix{jk} * (currentFV-prototypes(jk,:))')^2;
    end
    
    % Find the winning prototypes for this example
    correct = find(LGMLVQ_model.c_w == currentLabel);
    incorrect = find(LGMLVQ_model.c_w ~= currentLabel);
