...
%  trainLab = [1;1;2];
%  LGMLVQ_model=LGMLVQ_train(trainSet,trainLab);
...
%   Regularizes the eigenvalue spectrum of psisqrtor containing the regularization for each matrix psis{i}.
...
p.addOptional('optimization', 'sgd', @(x)any(strcmpi(x,{'sgd','fminlbfgs'})));
...
p.addOptional('Display', 'final', @(x)any(strcmpi(x,{'off','off'})));
p.addOptional('GradObj', 'on', @(x)any(strcmpi(x,{'on','off'})));
p.addOptional('HessUpdate', 'steepdesc', @(x)any(strcmpi(x,{'lbfgs', 'bfgs', 'steepdesc'})));
...
%                     dist(j) = (xi - model.w(j,:))*model.psis{j}'*(model.psis{j}*(xi - model.w(j,:))');
    ...
    %         disp([J,K,dJ,dK]);
            ...
            if epsilons(epoch)>0, % epoch >= MatrixStart
                ...
                % normalization % change by axp 27/05
                %disp("axp Normalizing...");
                for j=1:length(model.c_w)
                    model.psis{j} = model.psis{j} / sqrt(sum(diag(model.psis{j}'*model.psis{j})));
                end
            end
        end
        if nout>=2,
